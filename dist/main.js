"use strict";
const read = () => {
    const input = document.getElementById('grammar');
    const string_input = document.getElementById('string');
    const productionRules = input.value.split("\n").map(s => s.trim()).filter(Boolean);
    const string_value = string_input.value.trim();
    const steps = MainLogic(hashThis(productionRules), string_value);
    renderTable(steps);
};
const hashThis = (productionRules) => {
...
const MainLogic = (hashMap, inputString) => {
    var _a;
    const steps = [];
    const startSymbol = ((_a = hashMap.keys().next().value) !== null && _a !== void 0 ? _a : "");
    const prodList = [];
